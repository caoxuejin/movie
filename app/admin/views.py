...
def movie_add():
    ...
    if form.validate_on_submit():
        ...
        file_logo = secure_filename(form.log.data.filename)
        ...
        form.log.data.save(app.config["UP_DIR"] + logo)
        movie = Movie(
            title=data["title"],
            url=url,
            info=data["info"],
...
def movie_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if form.log.data.filename != "":
            file_logo = secure_filename(form.log.data.filename)
            movie.log = change_filename(file_logo)
            form.logo.data.save(app.config["UP_DIR"] + movie.log)
...
# 添加预告
@admin.route("/prerview/add/", methods=["GET", "POST"])
@admin_login_req
def preview_add():
    form = PreviewForm()
    if form.validate_on_submit():
...
# 修改预告
@admin.route("/prerview/edit/<int:id>/", methods=["GET", "POST"])
@admin_login_req
